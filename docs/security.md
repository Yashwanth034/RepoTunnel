...
- structured filesystem tools remain confined to approved workspace roots; AI live terminal/process commands are also confined by the current native OS sandbox (Bubblewrap on Linux, AppContainer on Windows, or the macOS Seatbelt compatibility backend), with only narrow authenticated Git/GitHub passthrough operations outside it
...
The **disposable verification sandbox** is limited to discovered build/test/check/lint presets and has an independent per-workspace `review`, `automatic`, or `disabled` policy. It requires the native sandbox for the current OS and probes that backend before execution. Presets run with network disabled, stdin closed, a sanitized environment, bounded time/output, and a disposable copy of the approved project. Protected secret paths and symlinks are omitted from that copy. Dependency/toolchain access is narrowed so credential files from the user's home directory are not exposed. Writes in this path are discarded with the temporary copy.

The **AI live terminal/process surface** operates on the real approved project through a fail-closed native OS boundary. Linux retains the existing Bubblewrap namespace/mount implementation. Windows uses an ephemeral AppContainer identity plus a kill-on-close Job Object and command-lifetime project ACL grants with crash-recovery cleanup. macOS uses a per-process Seatbelt profile through the deprecated `sandbox-exec` compatibility interface; a signed App Sandbox/XPC helper remains the long-term macOS backend. Credential-like environment overrides remain refused, and the sandbox backend is never silently replaced with unrestricted host execution. Live commands may use network access while disposable verification remains network-denied.

Windows AppContainer network isolation blocks host loopback by default. RepoTunnel does not silently add an administrator-level global loopback exemption. Internet/private-network capability can be granted to a live sandbox, but workflows that specifically require an AppContainer process to communicate with an unpackaged host service over `localhost` need an explicit local loopback policy configured by the user/administrator.
